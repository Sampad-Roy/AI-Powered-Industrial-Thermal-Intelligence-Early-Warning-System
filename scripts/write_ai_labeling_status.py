...
# Write report
report_content = f"""# AI/ML Stage: Current Dataset & Labeling State Audit Report
...
+-------------------------------+-----------------------+---------------------------------------+
| Total Dataset Events          | {total_events} events           | 100.0%                                |
| Officially Validated Events   | {len(validated_events)} events            | {len(validated_events)/total_events*100:.1f}%                                 |
| Formally Audited for Senior   | 3 events              | 1.5% (Cluster 74 SOP Flagged)         |
| Pending Human Validation      | {len(needs_val_events)} events           | {len(needs_val_events)/total_events*100:.1f}%                                 |
+-------------------------------+-----------------------+---------------------------------------+
```

### Current Distribution of `final_label` in `HUMAN_LABELING_SHEET.csv`
| Label / Class Name | Status | Event Count | % of Dataset | Notes |
| :--- | :---: | :---: | :---: | :--- |
| **Persistent Industrial Heat** | `Validated` | **{len(validated_events)}** | **{len(validated_events)/total_events*100:.1f}%** | 25 from Cluster 4 + 4 from Cluster 3 |
...
| **Unknown** | `Needs Human Validation` | **{len(needs_val_events)}** | **{len(needs_val_events)/total_events*100:.1f}%** | Awaiting structured human review |
...
| Cluster Grouping | Total Events | Current Label Status | Verified Target Class | Evidence Summary |
| :--- | :---: | :---: | :--- | :--- |
...
1. **Supervised Training Block**: Machine learning models (Random Forest, XGBoost, LightGBM, MLP, SVM) must **NOT** be trained until a balanced, ground-truthed representation of all 4 classes is validated.
...
print(f"AI_LABELING_STATUS.md successfully created at {report_file}")
