...
export default function EventTable() {
  ...
  return (
    <div className="bg-[#050b17] border-t border-[#16233b] flex flex-col transition-all duration-300 z-10 shrink-0 shadow-2xl">
      {/* Header bar with collapse toggle */}
      <div
        onClick={() => setCollapsed(!collapsed)}
        className="px-4 py-2 bg-[#091222] border-b border-[#16233b] flex items-center justify-between cursor-pointer hover:bg-[#0e1b32] transition-colors select-none"
      >
      ...
      {!collapsed && (
        <div className="max-h-56 overflow-y-auto overflow-x-auto bg-[#050b17]">
          <table className="w-full text-left border-collapse text-xs select-none">
            <thead>
              <tr className="bg-[#081020] border-b border-[#16233b] text-[10px] font-mono uppercase tracking-wider text-slate-400 sticky top-0 z-10 shadow-sm">
                <th
                  onClick={() => handleSort('event_id')}
                  className="px-3.5 py-2.5 cursor-pointer hover:text-cyan-300 transition-colors"
                >
                ...
                <th className="px-3.5 py-2.5">LOCATION (OSM)</th>
                <th
            ...
            <tbody className="divide-y divide-[#101b2f] font-mono">
              {sortedEvents.length === 0 ? (
                <tr>
                  <td colSpan={11} className="py-8 text-center text-slate-400 font-mono text-xs">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <Layers className="w-6 h-6 text-slate-400 opacity-60" />
                ...
                return (
                  <tr
                    ...
                    className={`cursor-pointer transition-colors ${
                      isSelected
                        ? 'bg-[#0d223d] text-cyan-200 border-l-4 border-l-cyan-400'
                        : 'hover:bg-[#0a1426] text-slate-300'
                    }`}
                  >
                    <td className="px-3.5 py-2 font-bold text-cyan-400 font-mono tracking-tight">{ev.event_id}</td>
                    <td className="px-3.5 py-2 max-w-[200px]">
                    ...
                    <td className="px-3.5 py-2 font-bold text-amber-300 font-mono text-sm">{ev.final_risk_score}</td>
                    <td className="px-3.5 py-2">
                      <span
                    ...
                    <td className="px-3.5 py-2 text-right">
                      <button
                        ...
                        className="btn-tactical text-[10px] px-2.5 py-1 bg-[#0f1d35] hover:bg-cyan-950 text-cyan-300 border-cyan-800 hover:border-cyan-400 font-mono font-semibold transition-all hover:scale-105 shadow-sm"
                      >
                        INSPECT
                      </button>
                    </td>
